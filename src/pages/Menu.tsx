...
const subcategoryOrder: Subcategory[] = [
  "all",
  "coffee",
  "tea",
  "others",
...
export default function Menu() {
  ...
  // Subcategory filter only makes sense inside Beverages;
  // reset it whenever the person leaves that category.
  useEffect(() => {
  ...
  const handleAddToCart = (
    ...
    toast.success(
      `${getDishName(dish)} ${t("menu.addedToCart")}`
    );
  };
  ...
  return (
    <main className="bg-table-dark min-h-screen pt-[72px]">
      ...
      >
        ...
        {/* Spacing when there are no subcategories */}
        {activeCategory !== "beverage" && (
          <div className="mb-10" />
        )}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse bg-table-mid rounded-lg h-80"
              />
            ))}
          </div>
        ) : isError ? (
                        ...
                        {dish.category === "beverage" &&
                          dish.subcategory && (
                            <span className="px-3 py-1 text-xs text-cream/60 border border-cream/15 rounded-full">
                              {t(
                                `menu.subcategories.${dish.subcategory}`
                              )}
                            </span>
                          )}
                      </div>
