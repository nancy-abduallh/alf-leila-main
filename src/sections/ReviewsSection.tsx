            ...
            <div className="max-w-[1000px] mx-auto px-6 lg:px-12">
                ...
                {customer && (
                    <form onSubmit={handleSubmit} className="bg-table-mid border border-gold-primary/10 rounded-lg p-6 mb-12 max-w-xl mx-auto">
                        <p className="text-cream text-sm mb-3">{myReview ? t("reviews.updateReview") : t("reviews.leaveReview")}</p>
                        ...
                        >
                            {submitReview.isPending ? t("reviews.submitting") : myReview ? t("reviews.update") : t("reviews.submit")}
                        </button>
                    </form>
                )}
                        ...
                        {reviews.map((review) => (
                            <div key={review.id} className="bg-table-mid border border-gold-primary/10 rounded-lg p-6">
                                ...
                                <p className="text-cream/40 text-xs mt-4">
                                    {review.userName || t("reviews.guest")} · {new Date(review.createdAt).toLocaleDateString()}
                                </p>
                            </div>
                        ))}
                    </div>
                ) : (
                    <p className="text-cream/40 text-center">{t("reviews.noReviews")}</p>
                )}
            </div>
        </section>
    );
